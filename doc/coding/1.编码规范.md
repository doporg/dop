...
>在阅读前建议阅读《阿里巴巴Java开发手册》

**！！！！！！！所有函数必须有注释！！！！！！！**

* controller中的API接口必须说明每个参数的含义并通过swagger api写明对应的注解，如下：
...
```java
   /**
    ...
    public Pagination<UserV1> getUserPaginationV1(@ApiParam(name = "pageNo",value = "页号",required = true,defaultValue = "1") @RequestParam(value = "pageNo", required = false, defaultValue = "1") Integer pageNo,
                                                  @ApiParam(name = "pageNo",value = "页号",required = true,defaultValue = "10") @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize) {
        return this.userService.getUserV1Pagination(pageNo, pageSize);
    }
```

* service层，若参数与controller层变化不大且业务简单，可只写一句注释。否则需和上面一样写详细注释。
* dao层，一句话注释并说明每个参数含义
...
* Manager层：通用业务处理层。代码在对应manager包中
  ...
  * 与DAO层交互，对多个DAO的组合复用。代码在对应dao包中
