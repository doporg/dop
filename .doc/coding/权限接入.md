...
D:\Work\dop\permission-server\src\main\java\com\clsaa\dop\server\permission\service\AuthenticationService.java
...
**此文件通过feign调用接口，使用时可以直接复文件AuthenticationService和DTO层的RoleDtoV1和UserRuleDtoV1，如果还需要其他功能，我后续再增加API**
...
函数原型： 
 ...
 例子：
       ...
       public Long createRole(Long parentId,String name, Long cuser,Long muser)
        {
            if(authenticationService.checkUserPermission("创建角色",cuser)) //调用校验函数
            {
...
调用 authenticationService 中的 findAllIds函数：
...
函数原型： 
 ...
 以下为具体例子：
    注意：如果是分页查询，需要改一下分页查询的方法，要自己手动进行分页操作，而不是直接在调用DAO层函数时分页
...
## 2.4 authenticationService.java 文件
