...
* mtime:修改时间，类型为LocalDateTime（若记录不可能被修改则此字段可删除）
* cuser:创建人，类型为Long（若不存在创建人则此字段可删除）
* muser:修改人，类型为Long（若不存在修改人则此字段可删除）
* ouser:所属人, 类型为Long（若没有修改所属人等相关业务逻辑则不需要此字段）
* status:状态，类型为enum，在数据库中设置为varchar
* type:类型,类型为enum，在数据库中设置为varchar
* deleted:删除标记，用于逻辑删除，类型为boolean，数据库 字段名为is_deleted
* orgnizationId，组织id，类型为Long（一般都会包含此字段）
