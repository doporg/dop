...
public interface ProjectRepository extends JpaRepository<Project, Long> {
     ...
     * @param status 项目状态
     * @param title  搜索字符串
     * @return {@link List<Project>} 项目持久层对象
     */
    //@Query(value = "select p from Project p where p.status=:status and p.searchString like :searchString % ORDER BY ?#{#pageable}",
    //        countQuery = "SELECT count(*) FROM Project WHERE STATUS=:status")
    List<Project> findAllByStatusAndTitleStartingWith(Project.Status status, String title);
    ...
    List<Project> findAllByTitleStartingWith(String Title);
